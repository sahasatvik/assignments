...
\begin{document}
    ...
    \begin{theorem}[Heisenberg's Uncertainty Principle]
        ...
        \] 
    \end{theorem}
    Consider the following expression, which we evaluate using integration by parts.
    \begin{align*}
        i\hbar \int \ddx{\psi^*}\psi\:dx = -i\hbar\int \psi^*\psi\:dx - i\hbar \int
        \ddx{\psi}\psi^*\:dx = -i\hbar + \left[i\hbar \int \ddx{\psi^*}
        \psi\:dx\right]^*
    \end{align*}
    Thus, \[
        2i\operatorname{Im}\int i\hbar \ddx{\psi^*}\psi \:dx = -i\hbar. 
    \] Since $z^*z \geq b^2$, where $z = a + ib$, we must have \[
        4\left[\int i\hbar \ddx{\psi^*}\psi\:dx\right]^2 \geq \hbar^2.
    \] Cauchy Schwarz gives \[
        \int x\psi^*\:x\psi \:dx \;\int
        \left(i\hbar\ppx{\psi^*}\right)\left(-i\hbar\ppx{\psi}\right) \:dx \geq 
        \left[\int i\hbar \ddx{\psi^*}\psi\:dx\right]^2 \geq \frac{\hbar^2}{4}.
    \] In other words, \[
