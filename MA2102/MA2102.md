...
- **Zero/trivial map**: $0\colon V\to W$, $\v \mapsto \ve0$.
- **Identity map**: $\Id\colon V\to V$, $\v \mapsto \v$.
- **Scaling**: $T\colon \R\to\R$, $x \mapsto cx$ for some fixed $c$.

> Indeed, *all* linear maps from $\R$ to $\R$ are scaling maps.

- **Dilation**: $T\colon V\to V$, $\v \mapsto c\v$ for some fixed $c \in F$.
- **Matrices**: $L_A\colon \R^n\to\R^m$, $\v\mapsto A\v$ for some $A \in M_{m\times n}(\R)$.
...
- **Inclusions**: For example, $T\colon \R\to\R^2$, $x\mapsto(x, mx)$.