...
$(\Rightarrow)$ Let $S\cup\{\v\}$ be linearly independent. Suppose that $\v \in \span S$, *i.e.* for some $\v_i \in S \cup \{v\}$, $\lambda_i \in F$,
...
- The set of polynomials $\beta = \{1, x, x^2, \dots\}$ is a basic of the space of real polynomials $P(\R)$.
...
for scalars $\lambda_i, \mu_i \in F$. Thus,
