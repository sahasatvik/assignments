...
The Cauchy criterion for the sequence $\{s_k\}$ implies that the series $\sum_{n = 1}^\infty \alpha_n$ converges *iff* for every $\epsilon > 0$, there exists and integer $N$ such that for all $n > m \geq N$, we have
