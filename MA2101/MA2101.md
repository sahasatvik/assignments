...
Note that every neighbourhood of a limit point of $S$ contains infinitely many points of $S$. As a corollary, finite subsets of a metric space have no limit points.
...
A set $S$ is open *iff* its complement is open. As a corollary, a set is open *iff* its complement is closed.