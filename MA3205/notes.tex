...
\begin{document}
    ...
    \begin{definition}
        Let $\gamma\colon (a, b) \to \R^n$ be a regular curve. Let $\Delta s$ be the
        length of the curve from $\gamma(t)$ to $\gamma(t + \Delta t)$, and let
        $\Delta\theta$ be the angle between these two vectors. Then, the curvature of
        $\gamma$ at $\gamma(t)$ is defined as \[
